export const portfolioProjectShowcasePage = [
    {
        id: 1,
        key: "portfolioProjectShowcasePageId1",
        header: "Design Bundle",
        text: "Morbi accumsan ipsum velit. Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non mauris vitae erat consequat auctor eu in elit. Class aptent taciti sociosqu ad litora torquent conubia nostra per inceptos himenaeos.",
        categories: [
            {
                id: 1,
                label: "Art Direction",
                key: "artDirection",
                isHover: "init",
                path: "portfolio-category/art-direction"
            }
        ],
        date: "26.02.2021",
        tags: [
            {
                id: 1,
                key: "creative",
                tagName: "Creative",
                isHover: "init",
                active: "init"
            },
        ],
        imagesArray: [
            {
                id: 1,
                key: "portfolioProjectShowcasePageId1Img1",
                imageName: "freestocks-A11MXTzUhLE-updated-unsplash.png"
            },
            {
                id: 2,
                key: "portfolioProjectShowcasePageId1Img2",
                imageName: "stil-flRm0z3MEoA-updated-unsplash.png"
            },
            {
                id: 3,
                key: "portfolioProjectShowcasePageId1Img3",
                imageName: "uby-yanes-393QQ30ecvQ-updated-unsplash.png"
            }
        ],
        backgroundImage: {
            id: 1,
            key: "portfolioProjectShowcasePageId1BackgroundImg1",
            imageName: "uby-yanes-LMwW0ARHc4o-updated-unsplash.png",
            style: {
                width: 70,
                transition: 0,
                rendered: false
            },
            animation: false
        }
    },
    {
        id: 2,
        key: "portfolioProjectShowcasePageId2",
        header: "Man Collection",
        text: "Morbi accumsan ipsum velit. Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non mauris vitae erat consequat auctor eu in elit. Class aptent taciti sociosqu ad litora torquent conubia nostra per inceptos himenaeos.",
        categories: [
            {
                id: 1,
                label: "Art Direction",
                key: "artDirection",
                isHover: "init",
                path: "portfolio-category/art-direction"
            }
        ],
        date: "26.02.2021",
        tags: [
            {
                id: 1,
                key: "creative",
                tagName: "Creative",
                isHover: "init",
                active: "init"
            },
        ],
        imagesArray: [
            {
                id: 1,
                key: "portfolioProjectShowcasePageId2Img1",
                imageName: "chris-lee-70l1tDAI6rM-updated-unsplash.png"
            },
            {
                id: 2,
                key: "portfolioProjectShowcasePageId2Img2",
                imageName: "kara-eads-Uq9o9uUIT08-updated-unsplash.png"
            },
            {
                id: 3,
                key: "portfolioProjectShowcasePageId2Img3",
                imageName: "leone-venter-pVt9j3iWtPM-updated-unsplash.png"
            }
        ],
        backgroundImage: {
            id: 1,
            key: "portfolioProjectShowcasePageId2BackgroundImg1",
            imageName: "olena-sergienko-dIMJWLx1YbE-updated-unsplash.png",
            style: {
                width: 70,
                transition: 0,
                rendered: false
            },
            animation: false
        }
    },
    {
        id: 3,
        key: "portfolioProjectShowcasePageId3",
        header: "Branding & Website",
        text: "Morbi accumsan ipsum velit. Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non mauris vitae erat consequat auctor eu in elit. Class aptent taciti sociosqu ad litora torquent conubia nostra per inceptos himenaeos.",
        categories: [
            {
                id: 1,
                label: "Art Direction",
                key: "artDirection",
                isHover: "init",
                path: "portfolio-category/art-direction"
            }
        ],
        date: "26.02.2021",
        tags: [
            {
                id: 1,
                key: "creative",
                tagName: "Creative",
                isHover: "init",
                active: "init"
            },
        ],
        imagesArray: [
            {
                id: 1,
                key: "portfolioProjectShowcasePageId3Img1",
                imageName: "drew-beamer-ehBB9G7qbss-unsplash.png"
            },
            {
                id: 2,
                key: "portfolioProjectShowcasePageId3Img2",
                imageName: "lee-campbell-gA-km82BywY-unsplash.png"
            },
            {
                id: 3,
                key: "portfolioProjectShowcasePageId3Img3",
                imageName: "damian-patkowski--pahtnAMuFo-unsplash.png"
            }
        ],
        backgroundImage: {
            id: 1,
            key: "portfolioProjectShowcasePageId3BackgroundImg1",
            imageName: "workperch-AiOzVFqlyZI-unsplash.png",
            style: {
                width: 70,
                transition: 0,
                rendered: false
            },
            animation: false
        }
    }
]
